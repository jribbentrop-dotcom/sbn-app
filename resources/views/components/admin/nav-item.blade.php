...
<a href="{{ route($route) }}"
   class="sbn-nav-item {{ $isActive ? 'active' : '' }}"
   title="{{ $label }}">
    <span class="sbn-nav-icon">
        @switch($icon)
            @case('dashboard')
            ...
            @case('progression')
                ...
                @break
            @case('rhythm')
                <svg viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0017 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                @break
            @case('voicing')
    ...
    <span class="sbn-nav-text" x-show="!$store?.sidebarCollapsed ?? true" x-transition.opacity>
        {{ $label }}
        @if($badge)
            <span class="sbn-nav-badge">{{ $badge }}</span>
        @endif
    </span>
    @if($badge)
        {{-- Show badge as dot when sidebar is collapsed --}}
        <span class="sbn-nav-badge-dot" x-show="sidebarCollapsed"></span>
    @endif
</a>
