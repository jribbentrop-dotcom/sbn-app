...
### 2.3 Versions (arrangements) — work/arrangement split (shipped 2026-06-23–29)

A song can have multiple **arrangements** (difficulty levels, performer versions) under one library entry. Two-axis model — don't conflate them:

- **Axis A — arrangements** = different *data* for the same song (difficulty/performer). One list entry, a dropdown to switch, separate DB rows.
- **Axis B — notation layers** = two notated views *within one arrangement* (melody TAB vs chord/comping TAB). A viewer/editor toggle, not a dropdown — they share the arrangement's key/form/progressions.

**`sbn_leadsheets` = the WORK** (catalog identity): `slug`, `title`, `composer`, `song_key` (canonical/default), `time_signature`, `genre`, `popularity`, `status`, `description`/`harmony_notes`/`form_notes`/`voicing_notes`, `license_status`, `is_pro`, `default_version_id` (FK). **Licensing stays on the work** — `is_pro`/`license_status` must never live on a version, otherwise one arrangement could be PD and another not.

**`sbn_leadsheet_versions` = the ARRANGEMENT** — [`app/Models/LeadsheetVersion.php`](../app/Models/LeadsheetVersion.php), owns the arrangement-level accessors (`getParsedDataAttribute`, `getSectionCount`, `getHasMelody`, `getHasTabXml`, etc.):

```
id, leadsheet_id (FK, cascade), label, performer, difficulty (1–5),
version_slug (unique per leadsheet_id — URL key, stable across deploy_db.sh reseeds
              where autoincrement ids can drift local↔prod),
song_key, rhythm, tempo, measure_count,
json_data           ← chord grid + chordVoicings (the version's parsed_data)
melody_tab_xml      ← notated melody MusicXML
chord_tab_xml       ← notated comping/rhythm TAB MusicXML (Axis B — see admin editor ref §"Tab layers")
shortcode_content, sort_order, status ('draft'|'publish')
```

URL: `/library/songs/{slug}/viewer?v=joao-gilberto`. Falls back to `default_version_id` when `?v=` is absent.

**Version-scoped detection caches.** `sbn_progression_occurrences`, `sbn_voicing_usage`, `sbn_voicing_drafts` all carry both `leadsheet_id` (kept denormalized) and `version_id` — a version's progressions/voicing shapes are derived from *its* `json_data`, which can differ per arrangement (e.g. a reharmonized performer version). **Aggregate popularity counts stay `leadsheet_id`-keyed** (`COUNT(DISTINCT leadsheet_id)` in `ProgressionDetector::song_count` / `VoicingCrossref::usage_count`) — switching them to `version_id` would inflate counts ~N× per arrangement. Only display/filtering queries use `version_id`.

**Controller resolution** — `SongLibraryController` (`show`/`viewer`/`cinema`/`apiViewerData`/`apiSheet`) resolves the active version from `?v=` (else default), passes `versions[]` for the dropdown, and reads all `json_data`/tab fields off that version. `abortIfNotPro()` still reads `is_pro` off the work.

**Migration/legacy note:** old `json_data`/`tab_xml`/`rhythm`/`tempo`/`measure_count`/`difficulty` columns remain on `sbn_leadsheets` as a dual-read fallback; not yet dropped.

Editor-side version CRUD (clone/delete/merge) and the melody/chord tab-layer switching mechanism live in [SBN-Admin-Chord-Tab-Editor-Reference.md](SBN-Admin-Chord-Tab-Editor-Reference.md) ("LEADSHEET EDITOR PAGE" → header bar / tab layers). Full design history and open items: [SBN-Leadsheet-Versions-Plan.md](SBN-Leadsheet-Versions-Plan.md).
...
**Tab layer sub-toggle (Melody/Chords — shipped 2026-07-07).** Within `tab` mode, when the active version has a chord/comping TAB layer (`hasChordTab`), the Options menu shows a second radio row ("Tab layer": Melody | Chords) below Display. Backed by `tabLayer` ref (`'melody' | 'chord'`, not persisted — resets to melody each load). `SongLibraryController::viewer()`/`apiViewerData()` pre-parse `chord_tab_xml` server-side (`resolveChordLayerMelody()`) into a `chordLayerMelody` events array on the leadsheet payload — the two layers share one `sections`/`chordVoicings`/repeats/voltas skeleton (only the staff notes differ, see the admin editor's "Tab layers" section for the as-built rationale), so switching layers just swaps `melodyRef.value` between `jsonData.melody` and `chordLayerMelody` and calls `buildModel()` again; no second full tab model or save round-trip needed since the viewer is read-only. `hasChordTab` on the frontend is `!!leadsheet.hasChordTab && !!leadsheet.chordLayerMelody` (belt-and-suspenders — the toggle never appears for a version whose chord XML failed to parse into any events).

**Gotcha — `TabXmlParser::parse()`'s output is NOT the flat-note shape `buildModel()` expects.** The parser groups notes by tick into `TabEvent`-shaped objects (`{ tick, notes: [{string, fret, ...}], isRest, ... }` — the same shape `useTabModel` itself *produces*, not what it *consumes*). Feeding that straight into `melodyRef` makes `buildModel()` read `.string`/`.fret` off an object that only has `.notes[]`, silently producing empty/garbage events — the chord layer then falls through to rendering whatever melody data was last loaded, with no error. `resolveChordLayerMelody()`'s `flattenTabEventsToNotes()` un-groups each `TabEvent` back into one entry per note/rest before handing it to the frontend. `apiSheet?layer=chord` (shipped 2026-06-29) had the same bug — never exercised end-to-end until this toggle surfaced it — and now routes through the same flattener.
...
#### 6.3.1 Rhythm-pattern layer (Chords + Tab modes)
...
**Audio merge** — `LeadsheetViewer.vue`'s `loadAllEvents()` merges the pattern into the active mode's event stream in **Chords and Tab modes** (2026-07-07 — widened from Chords-only). Analysis mode plays the same chord audio as Chords mode but doesn't get the rhythm layer (it keeps the flat beat row, no product need identified yet). Tab mode's real note-accurate melody/comping is a distinct `voice` from the percussion-only rhythm layer (`'nylon'` vs `'percussion'`), so the two don't collide the way two pitched guitar parts would — this is why Tab was excluded from the original 2026-07-06 cut and safe to include now. `buildRhythmEvents(totalBeats, anchorBeat)` loops `rhythmPatternToEvents()` to cover the song, tagging events `voice: 'percussion'`; `totalBeats`/`anchorBeat` are derived from `model.value` (mode-independent), so the same pickup-bar anchoring applies unchanged to Tab mode.
...
- **Cinema intentionally has no rhythm-pattern playback:** Cinema is designed around video/backing-track audio (§11 in the Cinema project notes), not the synth rhythm layer — the §6.3.1 audio merge is classic-Viewer-only by product decision, not an oversight. Cinema's `StageHeroNow.vue` beat row got the same RhythmStrip visual treatment but is still driven by a flat placeholder array, not `leadsheet.rhythmData` (which Cinema's controller payload does include, unused so far) — leave as-is unless that decision changes.
- **Rhythm-pattern picking-mode songs get no audio layer (Chords or Tab):** `buildRhythmEvents()` silently skips `pickingMode` patterns (see §6.3.1) since they'd need the song's real chord-tone MIDI (`ctx.chordMidi`) wired in to sound right instead of generic open-string defaults — deferred rather than shipping an audibly-wrong "second guitar part."
