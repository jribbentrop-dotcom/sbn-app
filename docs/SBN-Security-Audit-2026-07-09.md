...
*(This section is written as direct instructions to whichever Claude Code session picks this up next. If you're that session: follow it in order. If you're a human reading this, it doubles as a changelog of what still needs doing.)*

**If your assigned task is finding #5 (the `LeadsheetController` split):** you don't need to do Steps 1–4 below first, and you don't need to merge to `main` first either — continue directly on branch `claude/security-patch-d0q1nn` (checkout/pull it, verify you're on it with `git branch --show-current`). Skip straight to **finding #5** further down this doc for the concrete, ordered plan (extract the cross-cutting helpers first, decide `createFromLookup`'s home, then split, then verify). It also explains why this one specifically needs a real `sbn.db` to verify safely — read that before starting.
...
Check the Summary table below for current status, but in short, only two findings have no code fix left: **#3** (needs someone with production server access to confirm `APP_DEBUG=false` on the live `.env` — not verifiable from any sandbox) and **#10** (run the full `php artisan test` suite locally to establish the green/red baseline — this sandbox's subset already passes). **#5**'s two safe pieces are done and verified; the real 4-way `LeadsheetController` split around `createFromLookup`'s dual CRUD/transcription personality is still open if you want to keep going, see finding #5 below — everything else (#1, #2, #4, #6, #7, #8, #9) is fully fixed. Ask the user which of these they want next rather than assuming.
...
>
> **Still open — assigned to the next session.** The real split (Transcription is already its own service; what's left is Crud/Voicing/Progression). Concrete plan, in order:
>
> 1. **Extract the three cross-cutting helpers first**, before touching controller structure — they're why a naive split would just recreate the coupling across controllers instead of removing it:
>    - `normalizeChordNamesInJson` (private, ~line 2809 pre-split-numbering — re-`grep` it, line numbers have moved) — 9 call sites spanning CRUD (`store`, `update`), Transcription-adjacent (`reshiftDownbeat`, `redetect`, `reassembleFromRawResult`), and Progression-application (`applyProgression`, `fillVoicings`, `applyRhythm`).
>    - `serializeLeadsheet` + `backfillFingersFromCrossref` (always called as a pair) — CRUD (`apiShow`) + Transcription-adjacent (`reshiftDownbeat`, `retuneDetection`, `reassembleFromRawResult`). `backfillFingersFromCrossref` also has its own private helper cluster (`findCanonicalFingersForFrets`, `classifyFretMatch`, `mapCanonicalFingersToLeadFrets`, `fretStringToArray`, `canonicalFretsFromDiagram`, `canonicalFingersFromDiagram`) that should move with it.
>    - Move these into a new `App\Services\LeadsheetSerializer` (or similar) — same pattern as the `TranscriptionAssembler` extraction: pure move, inject into whichever controller(s) need it, verify nothing else references the old private methods.
> 2. **Decide `createFromLookup`'s home.** It's simultaneously a CRUD-create action and a full audio-transcription pipeline (YouTube/local upload → Demucs → basic-pitch → `assembleTranscription`). Either it stays on the CRUD controller and calls into `TranscriptionAssembler`/`MidiTranscriptionService` for its audio branch (probably the pragmatic choice — it's fundamentally "create a leadsheet," audio is just one input mode), or it gets its own controller. Don't force a split that fights the method's actual shape.
> 3. **Then split what's left**, following the clusters already mapped: `LeadsheetCrudController` (CRUD table: `index`/`create`/`store`/`update`/`destroy`/`edit`/`apiShow`/version-merge/song-merge/transpose/etc.) and `LeadsheetVoicingController` (voicing/diagram search + `applyProgression`/`fillVoicings`/`applyRhythm`/`applyRhythmToExercise`, which share the injected `LeadsheetParser`). Update `routes/web.php` — both the `admin` web group (~18 routes) and `api/admin` group (~22 routes) reference `LeadsheetController::class` directly and need repointing.
> 4. **Verify with the strong check this session learned the hard way**, not just `php -l`/`route:list`: reflect every touched controller's method parameter types and confirm they resolve, then `app()`-resolve each controller through the container. A missing `use` import on a FormRequest class caused exactly this kind of silent-until-runtime failure earlier in this project (see finding #7) — a class split has the same failure mode for model/service references.
> 5. **This one genuinely needs the real `sbn.db`.** Unlike #7 and the Transcription extraction (verifiable with pure/synthetic-data tests), a CRUD split touches `Leadsheet::create/update/delete` and real query chains — don't consider it done without running the existing DB-backed Feature tests (`LeadsheetLookupTest`, `LeadsheetProgressionTest`, etc.) against the real dev DB, which means this has to happen in an environment with `sbn.db` available (local, not this remote sandbox).
...
### 7. Thin request validation — Medium — ✅ Fixed 2026-07-14

> **Resolved.** Every admin controller in the codebase (23 total, 17 of which had write endpoints taking raw `Request`) now uses dedicated FormRequests for anything that validates/mutates. Started 2026-07-13 with the three endpoints this finding named (`updateIsPro`, `updateStatus`, `uploadBackingTrack` on `Admin/LeadsheetController`), which also closed a real gap: the old `updateIsPro` docblock claimed to guard against enabling `is_pro` on non-`public_domain` rows, but no such check existed — an instructor could set `is_pro=true` on a copyrighted song and expose it via the pro Viewer/Cinema path (the exact impact flagged in finding #1). Completed 2026-07-14 across the rest: `ChordController`, `ExerciseController`, `CourseController`, `LessonController`, `ProgressionController`, `PdfDocumentController` (its `update()` had **zero** validation at all — untrusted `title`/`status`/`content` written straight from `$request->input()`, another real instance of this finding, not just a refactor), `RhythmPatternController`, `ProductController`, `SkillNodeController`, `ProgressionBuilderController`, `Admin/CommunityController`, `Admin/MessageController`, `CourseGrantController`, `AdminFretboardController`, `AIController`, and finally the remaining 16 methods in `LeadsheetController` itself (the largest single chunk — the audio-transcription pipeline: `createFromLookup`'s ~30-field multi-mode validation, `separateStems`, `redetect`, `reshiftDownbeat`, `retuneDetection`, `transcribeStem`, etc.).
>
> All new FormRequests `authorize()` via `$user->isInstructor()` as defense in depth on top of the route-level `instructor` middleware (every route touched was already inside the `auth+instructor` group — this closes finding #7, it doesn't re-close #1). Business-rule checks that were previously manual post-`validate()` throws (`ValidationException::withMessages(...)` after the fact) were moved into each FormRequest's `withValidator()`/`after()` hook where that was the faithful equivalent — e.g. the video-snippet end-after-start/≤16-bar cap in `ChordProgressionRequest`/`RhythmPatternRequest`, the `content_json`-must-decode-to-array check in `ExercisePayloadRequest`. Verified with `grep -c '\$request->validate(' app/Http/Controllers/Admin/*.php` returning 0 everywhere, plus (after catching a real bug this way, see below) a reflection sweep of every controller's method-parameter types + actually resolving all 23 admin controllers through the container (`app($controller::class)`), not just `php -l`/`route:list`.
>
> **Caught a real bug doing this**, worth calling out: after committing 8 FormRequest conversions with `route:list` as the only check, a stronger verification (reflecting each method's parameter types and confirming they resolve, then `app()`-resolving the controller) found 8 methods referencing FormRequest classes with no `use` import — valid PHP syntax (different namespace, so it silently resolved the bare class name relative to the controller's own namespace instead), and `route:list` doesn't catch it either since it never resolves parameter types, only introspects route definitions. Every one of those 8 routes would have fataled with a class-not-found error the instant they were hit. Fixed immediately, and the stronger check became standard practice for the rest of this finding.

Only 3 `FormRequest` classes existed for a large admin write surface before this. Most write endpoints took a raw `Request`. This paired naturally with fixing #1.
...
**2026-07-14, continued — #7 completed.** Went through all 23 admin controllers converting every raw-`Request` write endpoint to a dedicated FormRequest — see finding #7 above for the full list and the real bugs this surfaced (`PdfDocumentController::update()` had zero validation; a missing-`use`-import bug that would have fataled 8 routes at runtime, caught by reflection + container resolution, not `route:list`). That verification approach (reflect every controller method's parameter types, then `app()`-resolve the controller) is worth reusing for any future controller surgery in this codebase — `php -l` and `route:list` both missed the import bug.

Remaining follow-up work:

1. **#3** — someone with production server access should confirm the live `.env` actually has `APP_DEBUG=false`.
2. **#5** — the real 4-way controller split (Crud/Voicing/Progression, plus resolving `createFromLookup`'s dual CRUD/transcription personality) is still open; the safe pieces are done.
3. **#10** — run `php artisan test` locally (with the real `sbn.db`) to establish the full green/red baseline. From this sandbox: all of `tests/Unit` plus `DetectionFilterTest`/`AudioContextRerankTest` pass except pre-existing, unrelated failures (`RhythmMaterializerTest`, 5 cases — untouched by any of this work) and DB-connection failures from tests that need the real `sbn.db` (expected, it isn't here). This is now the last audit item with no code fix — just needs the full-suite run.
