...
Completed: **#1** (instructor guard), **#2** (deleted dead route file), **#4** (synced `.env.example`), **#5** (split `LeadsheetController`), **#6** (shared `ChordQualityMapper`), **#7** (FormRequests across the entire admin write surface), **#8** (`UserProfile` fillable allowlist).
...
2. **#6 — DONE (2026-07-18).** The four quality-mapping methods were extracted into a shared `App\Services\Harmony\ChordQualityMapper` (`normalizeAlias`/`toChordNameSuffix` for the display path, `normalizeForFunction`/`toRomanSuffix` for the functional path); `ProgressionBuilder` and `ProgressionDetector` now delegate to it via thin private wrappers, call sites unchanged. The original concern was that the two copies had "diverged by design" and couldn't be merged mechanically — but by the time this was executed the four method **bodies were byte-identical** in both services (the two design paths were already correctly split *within* the mapper: display keeps `maj6`→`6`, functional collapses it to `maj7`), so it was a behavior-preserving extract. Locked in by `tests/Unit/ChordQualityMapperTest.php`; the Identifier suite (54 tests, 5941 assertions) exercises the delegated paths. **Residual, still open (not #6, tracked here so it isn't lost):** the `'+'` vs `'aug'` augmented-chord spelling divergence lives in the *parse-alias* code that surrounds these methods (`ProgressionBuilder` lines ~1367/1895/2078/3095, `ProgressionDetector` line ~137), NOT in the extracted suffix methods — so it was out of scope for the extraction. Still harmless (no canonical progression pattern references augmented chords) but worth deciding a canonical spelling before anyone touches that parse code.
